...
let absoluteDistances (tree: Tree<'a*float>) : List<List<float>> = 
    let rec absoluteDistances' (t: Tree<'a*float>) v : List<List<float>> = 
        match t with
        | Node((a,f), []) -> [[f+v]] 
        | Node((a,f), d) -> [[f+v]]@List.fold (fun acc elem -> mapper acc (absoluteDistances' elem (f+v))) [] d
    absoluteDistances' tree 0

let tree = Node("A", [Node("B", [Node("C",[]);Node("D",[]);Node("E",[])]);Node("F",[Node("G",[Node("H",[]);Node("I",[]);Node("J",[])])])])
